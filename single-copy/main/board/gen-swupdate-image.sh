...
sed -i "s/.*RECOVERY_IMAGE_HASH.*/\t\t\tsha256 = \"$RECOVERY_IMAGE_HASH\";/" $BR2_EXTERNAL_SINGLE_COPY_MAIN_PATH/output/images/sw-description
MAIN_IMAGE_HASH=$(sha256sum $BR2_EXTERNAL_SINGLE_COPY_MAIN_PATH/output/images/rootfs.ext2.gz | cut -d ' ' -f 1)
sed -i "s/.*MAIN_IMAGE_HASH.*/\t\t\tsha256 = \"$MAIN_IMAGE_HASH\";/" $BR2_EXTERNAL_SINGLE_COPY_MAIN_PATH/output/images/sw-description
